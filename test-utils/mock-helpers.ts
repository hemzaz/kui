 ...
 */
export function createMockKubectlResponse(overrides?: Partial<any>) {
  return {
    kind: 'Table',
...
export function createMockCommandTree() {
  ...
  return {
    listen: vi.fn((route: string, handler: any, options?: any) => {
      listeners.set(route, { handler, options })
    }),
    find: vi.fn((command: string) => {
      return listeners.get(command)
    }),
    subtree: vi.fn((path: string) => {
 ...
 */
export function createMockExecOptions(overrides?: Partial<any>) {
  return {
    tab: {
      uuid: 'test-tab-uuid'
    },
